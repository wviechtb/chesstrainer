play <- function(lang="en", ...) {
   ...
   iswin <- isTRUE(.Platform$OS.type == "windows")
   gui <- tolower(.Platform$GUI)
   isonline <- isTRUE(curl::has_internet())
   ...
   defaults <- list(player="", seqdir="", seqdirpos=1, mode="add", selmode="score_random", timed=FALSE, timepermove=5,
                    ...
                    lichessdb="lichess", speeds="blitz,rapid,classical", ratings="1200,1400,1600,1800", liout=1, lisort=1, barlen=50, invertbar=FALSE,
                    token="", uselicache=TRUE, showlibar=TRUE, monthslicache=12, minfreq=0, minperc=0,
   ...
   if (!is.element(mode, c("add","test","play")))
      mode <- "add"
   timepermove <- max(1, timepermove)
   expval <- max(0, expval)
   target <- min(100, max(0, target))
   target <- round(target)
   multiplier <- min(1, max(0, multiplier))
   adjustwrong <- min(100, max(0, adjustwrong))
   adjustwrong <- round(adjustwrong)
   adjusthint <- min(100, max(0, adjusthint))
   adjusthint <- round(adjusthint)
   if (length(showeval) == 1L)
      showeval <- c(add=showeval, test=showeval, play=showeval, analysis=showeval)
   evalsteps <- max(2, evalsteps)
   evalsteps <- round(evalsteps)
   movestoshow <- max(0, movestoshow)
   movestoshow <- round(movestoshow)
   piecesymbols <- min(3, max(1, piecesymbols))
   ...
   cex.bot <- max(0.1, cex.bot)
   cex.eval <- max(0.1, cex.eval)
   cex.coords <- max(0.1, cex.coords)
   cex.matdiff <- max(0.1, cex.matdiff)
   cex.plots <- max(0.1, cex.plots)
   cex.glyphs <- max(0.1, cex.glyphs)
   cex.lichess <- max(0.1, cex.lichess)
   ...
   if (!is.na(sflim) && (sflim > 20 && sflim < 1320))
      sflim <- NA
   sflim <- min(3190, sflim)
   sflim <- round(sflim/10) * 10
   multipv1 <- max(1, multipv1)
   multipv2 <- max(1, multipv2)
   ...
   hintdepth <- max(2, hintdepth)
   hintdepth <- round(hintdepth)
   monthssfcache <- max(1, monthssfcache)
   difflen <- max(2, difflen)
   difflen <- round(difflen)
   diffmin <- max(2, diffmin)
   ...
   if (length(mar2) == 1L)
      mar2 <- rep(mar2,4)
   mar2 <- pmax(mar2,1)
   barlen <- min(100, max(10, barlen))
   barlen <- round(barlen)
   if (is.null(token))
      token <- ""
   monthslicache <- max(1, monthslicache)
   minfreq <- min(100, max(0, minfreq))
   minperc <- max(0, minperc)
   ...
   if (!dir.exists(configdir)) {
      ...
      settings <- list(lang=lang, player=player, seqdir=seqdir, seqdirpos=seqdirpos, mode=mode, selmode=selmode, timed=timed, timepermove=timepermove,
                       ...
                       mar=mar, mar2=mar2,
                       lichessdb=lichessdb, speeds=speeds, ratings=ratings, liout=liout, lisort=lisort, barlen=barlen, invertbar=invertbar, token=token, uselicache=uselicache, showlibar=showlibar, monthslicache=monthslicache, minfreq=minfreq, minperc=minperc,
   ...
   # read in the opening database

   openings <- read.delim(system.file("openings", "all.tsv", package="chesstrainer"))
      ...
      if (new.square) {
         ...
         if (identical(buttons[1], 0L) || click.num == 2) {
            if (isTRUE(pos.x == click1.x) && isTRUE(pos.y == click1.y)) {
               ...
               ###if (flip) {
               ###   .drawpiece(pos.x, pos.y, pos[9-click1.x,9-click1.y])
               ###} else {
               ###   .drawpiece(pos.x, pos.y, pos[click1.x,click1.y])
               ###}
   ...
   while (run.all) {
      ...
      # open Lichess window if liout is 2 and it is not already open

      if (liout == 2 && length(dev.list()) == 1L) {
         .drawliwin()
         dev.set(which=2L)
         eval(expr=switch2)
      }
      ...
      while (run.rnd) {
         ...
         while (input) {
            ...
            if (mode == "play") {
               ...
               if ((flip && sidetoplay == "w") || (!flip && sidetoplay == "b")) {
                  ...
                  if (contliquery && !nzchar(bestmove[[1]][1])) {
                     ...
                     res.li <- .liquery(pos, flip, sidetoplay, sidetoplaystart, i, isonline, lichessdb, token, speeds, ratings, liout, lisort, barlen, invertbar, minfreq, minperc)
                     bestmove <- res.li$selmove
                  }

                  if (!nzchar(bestmove[[1]][1])) {
                     if (identical(matetype, "none") && !threefold && !fifty)
                        .texttop(ifelse(contliquery, .text("nomoveli"), .text("nomovesf")))
                     mode <- "analysis"
                     assign("mode", mode, envir=.chesstrainer)
                     .textbot(show, showcomp, player, seqname, seqnum, opening, score, rounds, age, difficulty, i, totalmoves, selmode, k, seqno)
            ...
            if (identical(click, "q") || identical(click, "ctrl-Q")) {
               ...
               if (session.playtime > mintime) { # only save session if it was longer than <mintime> seconds
            ...
            # \ (or ä) to switch into play mode or to analysis mode from play mode

            if (identical(click, "\\") || identical(click, "\U000000E4")) {
               if (!contliquery && !sfrun) {
               ...
               if (compmove) {
                  res.sf <- .sf.eval(sfproc=sfproc, sfrun=ifelse(contliquery, FALSE, sfrun), depth=depth3, multipv=multipv1, sflim=sflim, fen=fen, compmove=TRUE)
                  evalval  <- res.sf$eval
            ...
            if (identical(click, "ctrl-R")) {
               .texttop(.text("rlydelplayer", player), assign=FALSE)
               answer <- getGraphicsEvent(prompt="Chesstrainer", consolePrompt="", onKeybd=.keyfun)
               answer <- .confirm(answer)
               if (answer) {
                  .texttop(.text("rlyrlydelplayer", player), assign=FALSE)
                  answer <- getGraphicsEvent(prompt="Chesstrainer", consolePrompt="", onKeybd=.keyfun)
                  answer <- .confirm(answer)
               }
               if (answer) {
            ...
            if (identical(click, "Right")) {
               ...
               if (i > nrow(sub$moves)) {
                  ...
                  circlesvar <- ""
                  arrowsvar  <- ""
                  if (nrow(circles) >= 1L || nrow(arrows) >= 1L || nrow(harrows) >= 1L || .isglyph(glyph)) {
                     .rmannot(pos, circles=circles, arrows=rbind(arrows, harrows), glyph=glyph, flip=flip)
                     if (nrow(circles) >= 1L)
                  ...
                  if (mode %in% c("add","analysis")) {
                     ...
                     res.sf <- .sf.eval(sfproc=sfproc, sfrun=sfrun, depth=depth3, multipv=multipv1, sflim=sflim, fen=fen, compmove=TRUE)
                     evalval  <- res.sf$eval
            ...
            # U to update all (selected) sequences ([u])
            ...
            if (mode == "test" && identical(click, "U")) {
               if (is.null(sfproc) || !sfrun) {
                  ...
                  .texttop(.text("rlyevalupdateall"), assign=FALSE)
                  answer <- getGraphicsEvent(prompt="Chesstrainer", consolePrompt="", onKeybd=.keyfun)
                  answer <- .confirm(answer)
                  if (answer) {
                     selmodeold <- selmode
            ...
            if (mode == "test" && identical(click, "B") && upsidedown) {
               ...
               difficulty <- .difffun(sub$player[[player]]$score, difflen, diffmin, adjusthint, multiplier)
               .textbot(show, showcomp, player, seqname, seqnum, opening, score, rounds, age=0, difficulty, i+1, totalmoves, selmode, k, seqno)
            ...
            if (identical(click, "a") || identical(click, "A")) {
               ...
               if (identical(click, "A")) {
                  ...
                  .drawboard(pos, flip=flip)
                  .drawcheck(pos, flip=flip)
                  .textbot(show, showcomp, player, seqname, seqnum, opening, score, rounds, age, difficulty, i=1, totalmoves, selmode, k, seqno)
                  circles <- matrix(nrow=0, ncol=2)
                  arrows  <- matrix(nrow=0, ncol=4)
                  harrows <- matrix(nrow=0, ncol=4)
                  evalvals <- NULL
            ...
            if (mode == "add" && identical(click, "z")) {
               ...
               .textbot(show, showcomp, onlyshow=TRUE)
               next
            }

            # Z to switch show computer moves on/off (only in add mode)
            ...
            if (mode == "add" && identical(click, "s")) {
               ...
               eval(expr=switch1)
               .texttop(.text("saveseq"), assign=FALSE)
               seqident <- sapply(dat.all, function(x) identical(sub$moves[1:5], x$moves[1:5]) && identical(sub$pos, x$pos))
               if (any(seqident))
               ...
               filename <- sub("\\.rds$", "", filename) # strip .rds from the end of filename
            ...
            ### toggles and keys related to getting position evaluations and Lichess queries
            ...
            # L to get the Lichess cloud evaluation of the current position

            if (mode %in% c("add","play","analysis") && identical(click, "L")) {
               if (!isonline) {
            ...
            if (identical(click, "Y")) {
               ...
               assign("contliquery", contliquery, envir=.chesstrainer)
               .texttop(.text("contliquery", contliquery), sleep=1)
               if (mode == "play")
                  .textbot(show, showcomp, player, seqname, seqnum, opening, score, rounds, age, difficulty, i, totalmoves, selmode, k, seqno)
               if (contliquery) {
                  .liquery(pos, flip, sidetoplay, sidetoplaystart, i, isonline, lichessdb, token, speeds, ratings, liout, lisort, barlen, invertbar, minfreq, minperc, showout=mode!="test", showlibar=mode!="test")
               } else {
            ...
            # _/+ to decrease/increase the sound volume

            if (identical(click, "_") || identical(click, "+")) {
               if (identical(click, "_")) {
                  volume <- round(max(0, volume - 25))
               } else {
                  volume <- round(min(100, volume + 25))
               }
               assign("volume", volume, envir=.chesstrainer)
               playsound(system.file("sounds", "move.ogg", package="chesstrainer"))
            ...
            if (identical(click, "ctrl-V")) {
               ...
               assign("verbose", verbose, envir=.chesstrainer)
               .texttop(.text("verbose", verbose), sleep=0.75)
               next
            }
            ...
            if (identical(click, ">")) {
               ...
               if (isTRUE(bookmark != "")) { # only TRUE if bookmark is neither NA nor ""
                  ...
                  # note: if 'bookmark' is NA, then we don't need to redraw, because nothing has been drawn by .bookmarks()
               }
               next
            }
            ...
            if (mode == "test" && identical(click, "ctrl-D")) {
               .texttop(.text("rlydelseq"), assign=FALSE)
               answer <- getGraphicsEvent(prompt="Chesstrainer", consolePrompt="", onKeybd=.keyfun)
               answer <- .confirm(answer)
               if (answer) {
                  .texttop(.text("delseq"), sleep=1, showlast=FALSE)
            ...
            # / or , to select one or more sequences, . to select the last saved sequence, | to search/show sequences based on their filename without selecting them
            ...
            if (mode %in% c("add","test") && (identical(click, "*") || identical(click, "8"))) {
               if (!is.null(selected)) {
                  .texttop(.text("allseqselected"), sleep=1, showlast=FALSE)
                  selected <- NULL
                  mode <- oldmode <- "add"
                  assign("mode", mode, envir=.chesstrainer)
                  .newround(seqno1=TRUE)
            ...
            if (identical(click, "F3")) {
               tab <- list(lang=lang, player=player, mode=mode, seqdir=seqdir[seqdirpos], selmode=selmode, zenmode=zenmode, timed=timed, timepermove=timepermove, expval=expval,
                           ...
                           #lichessdb=lichessdb, speeds=speeds, ratings=ratings, liout=liout, lisort=lisort, barlen=barlen, invertbar=invertbar, token=paste0(rep("*", nchar(token), collapse=""), uselicache=uselicache, showlibar=showlibar, monthslicache=monthslicache, minfreq=minfreq, minperc=minperc)
               .showsettings(tab)
               .redrawpos(pos, flip=flip)
               .drawannot(circles=circles, arrows=arrows, harrows=harrows, glyph=glyph, hint=TRUE, evalvals=evalvals, sidetoplay=sidetoplay)
               next
            ...
            if (identical(click, "F5")) {
               eval(expr=switch1)
               .vizsettings(cols.all, flip, show, showcomp, player, seqname, seqnum, opening, score, rounds, age, difficulty, i, totalmoves, sidetoplay, selmode, k, seqno, movestoplay, movesplayed, timetotal, timepermove, liout)
               eval(expr=switch2)
               .redrawall(pos, flip, show, showcomp, player, seqname, seqnum, opening, score, rounds, age, difficulty, i, totalmoves, sidetoplay, selmode, k, seqno, movestoplay, movesplayed, timetotal, timepermove)
               .drawevalbar(sub$moves$eval[i-1], i=i, starteval=starteval, flip=flip, showeval=showeval[[mode]])
               .drawlibar(.get("lasttotals"), flip=flip)
            ...
            if (identical(click, "F7")) {
               tmp <- .sfsettings(sfproc, sfrun, sfpath, usesfcache, depth1, depth2, depth3, sflim, multipv1, multipv2, threads, hash, hintdepth, monthssfcache)
               .redrawpos(pos, flip=flip)
               .drawannot(circles=circles, arrows=arrows, glyph=glyph)
               sfproc        <- tmp$sfproc
               sfrun         <- tmp$sfrun
            ...
            if (identical(click, "F8")) {
               tmp <- .lichesssettings(speeds, ratings, lichessdb, uselicache, liout, lisort, barlen, invertbar, minfreq, minperc, token, monthslicache, isonline)
               .redrawpos(pos, flip=flip)
               .drawannot(circles=circles, arrows=arrows, glyph=glyph)
               speeds     <- tmp$speeds
               ratings    <- tmp$ratings
            ...
            if (identical(click, "F9")) {
               ...
               if (!identical(seqdir, seqdirold) || !identical(seqdirpos, seqdirposold)) {
                  ...
                  if (selmodeold != selmode && !file.exists(file.path(seqdir[seqdirpos], ".sequential")))
                     .texttop(.text("selmodeswitch", selmode), sleep=1.5, showlast=FALSE)
                  .newround(seqno1=TRUE)
               } else {
                  .redrawpos(pos, flip=flip)
                  .drawannot(circles=circles, arrows=arrows, glyph=glyph)
            ...
            # ctrl-o to check if the FENs in the openings database (all.tsv) match those constructed when making the same moves

            if (identical(click, "ctrl-O")) {
               next # remove to be able to run this
               openings$fen <- ""
               flip <- FALSE
               .redrawall(pos, flip, show, showcomp, player, seqname, seqnum, opening, score, rounds, age, difficulty, i, totalmoves, sidetoplay, selmode, k, seqno, movestoplay, movesplayed, timetotal, timepermove)
               for (j in 1:nrow(openings)) {
                  print(j)
                  ...
                  fen <- .genfen(pos, flip, sidetoplay, sidetoplaystart="w", i)
                  openings$fen[j] <- .fenpart(fen, parts=1:4)
               }
               print(table(openings$epd == openings$fen))
               .newround()
               next
            }
         ...
         # if using <right> all the way to the end of a sequence, then i > nrow(sub$moves) which causes an error when trying to move a piece, so jump back to [b]

         if (mode == "test" && i > nrow(sub$moves) && !islastmove)
            next
         ...
         .drawglyph(glyph) # redraw the glyph since it may have been overdrawn by a rectangle or removed above when removing the annotations

         if (!islastmove) {

            # check that the move is legal; if it is not, then jump back to [b]
         ...
         if (mode %in% c("add","play","analysis")) {
            ...
            # if we are here, then the move is legal
         ...
         if (mode == "test") {
            ...
            if (i > nrow(sub$moves)) {
               ...
               .textbot(show, showcomp, player, seqname, seqnum, opening, score, rounds, age=0, difficulty, i, totalmoves, selmode, k, seqno)
               ...
               if (!replast && k > 1L) {
                  if (selmode %in% c("sequential","sequential_len","sequential_mov","age_oldest")) {
                     ...
                     if (dosave && selmode %in% c("score_random", "score_highest") && all(scores.selected < target) && anyabove) {
                        playsound(system.file("sounds", "finished.ogg", package="chesstrainer"))
                        .texttop(.text("belowtarget", target), sleep=2, showlast=FALSE)
                     }
                  }
               }
         ...
         if (mode %in% c("add","play","analysis")) {
            ...
            # here we are after the player move in add, play, or analysis mode
