.sessiongraph <- function(seqsplayed, mean.scores, playtime, lwd) {
   ...
   plot(NA, xlim=c(1,seqsplayed.total), ylim=c(min(round(unlist(mean.scores)))-1, max(round(unlist(mean.scores)))+1),
        xlab="", ylab=.text("meanscore"), bty="l", las=1, col.axis=.get("col.top"), col.lab=.get("col.top"), xaxt="n")
   axis(side=1, at=seq_len(seqsplayed.total), col.axis=.get("col.top"))

   start.pos <- 0

   for (j in 1:length(seqsplayed)) {
      segments(start.pos + 1, mean.scores[[j]][1], start.pos + seqsplayed[j], mean.scores[[j]][1], lty="dotted", col=.get("col.top"))
      points(start.pos + seq_len(seqsplayed[j]), mean.scores[[j]], type="o", pch=21, lwd=2, col=.get("col.square.l"), bg=.get("col.square.d"))
